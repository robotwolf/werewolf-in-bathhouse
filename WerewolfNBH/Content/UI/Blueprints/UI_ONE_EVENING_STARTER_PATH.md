...
These functions are not autogenerated by the scaffold. They are still manual Blueprint work.
...
The scaffold already created the named child widgets such as `Text_Label`, `Text_Value`, and `ProgressBar_Fill` in the meter-style widgets. What it did not create is the Blueprint graph function that drives them.
